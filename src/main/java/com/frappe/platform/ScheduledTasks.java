 ...
 * declares its task beans, in its {@code infrastructure}. Every due execution runs on exactly one instance, and one
 * that was running on an instance that died is taken over once its heartbeat expires.
 *
 ...
 *   <li>A failed execution is retried with exponential backoff ({@code frappe.scheduling.initial-backoff},
 *       doubling, {@code frappe.scheduling.max-retries} times). Afterwards recurring tasks continue on their schedule
 *       and one-time tasks keep retrying at the next backoff step, so no work is lost.
...
public interface ScheduledTasks {
     ...
     * the next execution follows it without a restart. Look it up or remove it by {@code TaskInstanceId.of(name,
     * entityId)} ({@code task.instance(entityId)} needs the schedule as data).
     *
     * @param name the task name, {@code <module>.<kebab-case-name>}
