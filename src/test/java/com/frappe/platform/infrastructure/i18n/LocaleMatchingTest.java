...
class LocaleMatchingTest {
    ...
    void matchesRegionalVariantsToTheirSupportedLanguage(String desired, String expected) {
        // When
        var match = LocaleMatching.all().match(Locale.forLanguageTag(desired));
    ...
    void matchesNothingForUnsupportedLanguages(String desired) {
        // When / Then
        assertThat(LocaleMatching.all().match(Locale.forLanguageTag(desired))).isEmpty();
    }
    ...
    void matchesTheBestWeightedLanguageOfAnAcceptLanguageHeader(String header, String expected) {
        // When
        var match = LocaleMatching.all().matchAcceptLanguage(header);
    ...
    void matchesNothingForAnUnsupportedEmptyOrMalformedAcceptLanguageHeader(String header) {
        // When / Then
        assertThat(LocaleMatching.all().matchAcceptLanguage(header)).isEmpty();
    }

    @Test
    void restrictedToEnabledLanguagesMatchesOnlyThose() {
        // Given
        var enabled = LocaleMatching.all()
    ...
    void restrictedToNoSupportedLanguageMatchesNothing() {
        // Given
        var enabled = LocaleMatching.all().restrictedTo(List.of(Locale.FRENCH));
