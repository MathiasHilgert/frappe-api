...
- HTTP server/client, JDBC (`datasource-micrometer`), connection pool, JVM, Spring Modulith module entries and cross-module listeners, NATS publishes (`nats.publish`, PRODUCER) and consumes (`nats.process`, CONSUMER, via `NatsProcessObservations`), outbox recovery (`outbox.recovery`, `outbox.redelivery`, gauge `outbox.dead.letters`), scheduled task executions (`scheduled.task`, tagged `scheduled.task.name` and `scheduled.task.outcome`; counter `scheduled.task.exhausted` for given-up one-time runs): spans and metrics come from the platform.
