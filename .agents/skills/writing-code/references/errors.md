...
- Stored events that no longer load (outbox rows of an older deployment): Spring Modulith 2.1.1 skips a row whose class is missing (WARN, no exception), and `EventPublication.getEvent()` throws Jackson 3's `JacksonException` for JSON that no longer fits the class. Handle it per row where the event is first read (`FailedPublicationResubmitter`, `NatsConnectSetup`) so one row never aborts a batch; catch `JacksonException`, not `RuntimeException`. No broad catch exists at the scheduler entry point: its only boundary catch is `DataAccessException`.
