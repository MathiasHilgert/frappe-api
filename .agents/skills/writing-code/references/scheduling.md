...
- Tenant-scoped work: the scheduler thread has no request and no tenant. An action that touches tenant-scoped tables binds the tenant of the entity it processes with `TenantScope` around each use case call (`tenants.runAs(run.tenantId(), () -> close.close(branchId))`), one tenant at a time, never inside a transaction (`persistence.md`, Tenancy and RLS); `platform.scheduled_tasks` itself is not tenant-scoped.
