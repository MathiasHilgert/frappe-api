...
Events carry ids only: an address or a code on an event would be persisted in the outbox, the archive and NATS. A notice listener reads the contact live through the owner's `Api` from the event's ids and passes `notification/<template>/<eventId>` as its idempotency key.

One-time codes never enter an event. The owning module's own listener generates the code, stores its hash, and hands it in memory to a required interface (SPI) declared in its root package; notification implements it and sends. identity's `CodeMailer` is the pattern (`notification.infrastructure.IdentityCodeMails`):

```java
@Component
class IdentityCodeMails implements CodeMailer {                    // com.frappe.identity.CodeMailer (SPI)
    ...
    @Override
    public void send(CodeMail mail) {                              // called by identity's listener, after the commit
        mailer.send(MailMessage.of(
                "notification/" + templateOf(mail.purpose()),      // <module>/<name>: sign-up-code, password-reset-code…
                mail.recipient(),
                mail.locale(),                                     // the recipient's language
                SupportedLocales.FALLBACK,                         // whole-message fallback: en
                Map.of("code", mail.code(), "minutes", mail.validFor().toMinutes())));
        // no idempotency key: every attempt carries a fresh code that replaces the earlier one
    }
}
```

The notice without a code (`sendAccountExists`) has the same content on every retry, so it passes `notification/account-exists/<reference>` (the triggering event's id). A `MailDeliveryException` propagates into identity's listener, which fails, and the outbox retries it.
...
- Every sentence comes from the module's catalogs through `texts.get(key, args...)`; never literals. The subject is the key `<module>.mail.<name>.subject`. Keys follow `i18n.md` (`notification.mail.code.expires`), in all three catalogs.
