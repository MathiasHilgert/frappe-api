...
- Each of those three tables carries a generated, indexed `event_id uuid` column (`V202609191200__add_event_id_to_outbox_tables.sql`), computed once as `(serialized_event::jsonb ->> 'eventId')::uuid` and never written by hand: `serialized_event` is JSON from Modulith's own `EventSerializer`, and `eventId` is a mandatory `DomainEvent` component, so it is always present under its record component name. The purge matches by equality on this column, not by scanning `serialized_event` with `LIKE`: at outbox scale a per-row text scan of three tables is an unbounded, unindexable cost that would hold the archive purge's transaction open far too long and compete with publishing (the "purging never blocks publishing" invariant).
