...
- Startup fails, naming the class and the fix, for a route without `@Access`, with two or more mapped methods, or outside a package ending in `.infrastructure.web`. Only `com.frappe` controllers are routes; framework controllers are not checked.
