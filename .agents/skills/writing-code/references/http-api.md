...
- Annotated route classes are the only way to serve a path. Startup fails for any `RouterFunction` bean and for any handler mapping that serves paths outside them (a custom mapping, a bean named `/…`); static resources are off (`spring.web.resources.add-mappings=false`). At runtime a request no route serves passes through to 404/405 only when no other handler mapping would serve it (defense in depth). Only `com.frappe` controllers are routes; framework controllers are not checked.
