...
A person may belong to several businesses, so the business is chosen per request by the path: business-scoped routes live under `/v1/businesses/{businessId}/…` (map `/businesses/{businessId}/tabs/{tabId}/close`). The platform's `BusinessPathFilter` runs after the posture is enforced and before any controller code, once per request, never cached across requests:

- A `{businessId}` that is not a canonical UUID answers 404 `not-found`.
- `AUTHENTICATED` route: the caller enters when its session is bound to that business (`STAFF`, `TERMINAL`, `GUEST`), or when it is a `PERSON` and the kernel port `BusinessMembership.isMember(personId, businessId)` (implemented by access; the default denies everyone) says yes. Everyone else gets 404 `not-found`, identical to an unknown business: 404 before 403, so a stranger never learns that a business exists, and a bound session never crosses into another business. A failing port answers the generic 500 (fail closed).
- `PUBLIC` route under the path: no membership check.
- Then the path's business is bound through `TenantScope` for the rest of the request, so every use case transaction sees only its rows (`persistence.md`). This is the only way a request binds a tenant; paths outside `/v1/businesses/` bind none. The binding is on the request thread: an asynchronous route (`Callable`, `DeferredResult`) must bind it itself.
