...
- Log assertions on the root logger are scoped to the threads doing the work (`com.frappe.CapturedLogs`: `fromCurrentThread()` for code run directly, `fromThreads(serverThreads())` for requests to the embedded server): cached contexts of other test classes keep running in the same JVM. A test that breaks its context's infrastructure for good (stops its NATS) closes the context with `@DirtiesContext`, or the context keeps reconnecting and logging for the rest of the run.
- Mail: `TestMailpitConfiguration` (Mailpit, `axllent/mailpit:v1.31`, fresh per context) registers `spring.mail.*` (Boot 4.1 has no Mailpit service connection); read what was sent through Mailpit's API (`/api/v1/search?query=to:<address>`, `/api/v1/message/<ID>`). Never call the real Resend API: stub `ResendEmails` with a `@Primary` bean.
- A test that leaves publications of a test-only listener failed runs on its own, non-reused database (`TestcontainersConfiguration.postgres(name)` and `connectionProperties`, see `MailOutboxIntegrationTests`): the recovery jobs of other cached contexts share the reused database and would dead-letter them as `UNKNOWN_LISTENER`.
