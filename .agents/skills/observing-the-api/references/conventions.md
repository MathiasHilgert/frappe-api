...
- Every `CommandBus#dispatch` and `QueryBus#ask` is one observation `use_case`: the timer `use_case` (Prometheus `use_case_seconds_count|sum|bucket`) gives rate, errors and duration per use case, and the span `<module> <UseCase>` wraps the handler including its commit.
- Tags: `use_case.name` (message type, e.g. `CloseTab`), `use_case.module`, `use_case.kind` (`command` | `query`), `outcome`, plus Micrometer's `error` (exception simple name, `none` otherwise). All bounded by the code base.
...
- Automatic spans: `http <method> <route>` (server/client), `connection`/`query`/`result-set` (JDBC), Spring Modulith module entries and cross-module listeners, `publish <subject>` (NATS, PRODUCER), `process <subject>` (NATS, CONSUMER), and `<module> <UseCase>` for every command and query dispatched through the bus.
