...
- Automatic spans: `http <method> <route>` (server/client), `connection`/`query`/`result-set` (JDBC), Spring Modulith module entries and cross-module listeners, `publish <subject>` (NATS, PRODUCER) and `process <subject>` (NATS, CONSUMER). Every scheduled task execution is `scheduled task <name>` (timer `scheduled.task`, tags `scheduled.task.name`, `scheduled.task.outcome`), a root span with the task's queries as children.
