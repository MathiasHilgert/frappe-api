...
- `DbSchedulerProperties` (`db-scheduler.*`): `table-name` (default `scheduled_tasks`, used verbatim in SQL, so `platform.scheduled_tasks` works), `polling-interval` 10s, `heartbeat-interval` 5m, `missed-heartbeats-limit` 6 (dead after 30m), `shutdown-max-wait` 30m (waited up to twice), `polling-strategy` FETCH, `failure-logger-level` WARN (`SchedulerBuilder.DEFAULT_FAILURE_LOG_LEVEL`; first recorded here as DEBUG, corrected in review round 2), `threads` 10.
...
- D3 RED `TaskHandlesTest.anEntityScheduleChangedConcurrentlyAsksForARetry` (the upsert returns false): no exception. GREEN: `EntityHandle.schedule` throws `TaskSchedulingException("… changed concurrently; set its schedule again")` (new message-only constructor); `EntityTask` Javadoc and `scheduling.md` updated. 10/10.
