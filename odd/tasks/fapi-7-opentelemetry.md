...
### T8
- After review round 1: `FRAPPE_TEST_DB=frappe_fapi_7 ./gradlew spotlessApply check --rerun-tasks`: BUILD SUCCESSFUL, 74 tests, 0 failures. Before it: 66 tests, 0 failures, javadoc doclint clean, `ModularityTests` green.
- PR not opened (orchestrator's decision); body per template from this document.

### Review round 1 (two reviews, changes requested; business-metrics scope confirmed as authorized by the human in Plane)
- MAJOR, recorder isolation (`18cebdb`): RED `BusinessMetricsRecorderTest.isolatesAThrowingDeclarationSoTheCallerAndOtherMetricsAreUnaffected` and `isolatesFailuresAfterCommitFromTheCommittingCaller` (the declared lambda's `IllegalStateException` reached the caller), `refusesToStartWhenAMetricNameIsAlreadyAnotherMeterType` (no startup check). GREEN: per-metric `catch (RuntimeException)` at the telemetry boundary with one WARN (`frappe.metric`, `frappe.event_type`, `frappe.event_id`), documented as the only allowed broad catch in `writing-code/references/errors.md`; meter types checked against the registry at startup; each series registered once and cached (`MeterKey`). Identity can only be pre-registered per name/type, not per tag value (values come from events).
- Minors in the same commit: `skipsNegativeAndNonFiniteMeasuredValues` (was already green: Micrometer drops negative samples; now skipped explicitly with a WARN, NaN included); fluent `measure` rejects `SECONDS` (RED `rejectsSecondsBecauseDurationsAreDeclaredOnDurationFields`); RED `rejectsAnEventOutsideAModulePackageWithAClearProblem` (was `StringIndexOutOfBoundsException` for an event directly in `com.frappe`); RED `ObservabilityConfigurationTest.neverRecordsSqlParameterValuesOnSpans` then `jdbc.datasource-proxy.include-parameter-values=false` explicit, "bind parameters, never literals with personal data" in the observability reference; comment on the `IDENTIFYING_KEY` snake_case assumption.
- Log correlation (`a26f4ae`): `RequestTracingTests.logLinesOfARequestCarryItsTraceAndSpanIds` now renders the captured event with the base ECS configuration and asserts `trace.id`/`span.id` equal the server span; the rendering lives in the shared `EcsLogRenderer`, also used by `TraceCorrelationLoggingTests`.
- Logs to Loki (`0024e0b`): Boot 4.1.1 auto-configures `SdkLoggerProvider` + OTLP log exporter (and compose/`otel-lgtm` connection details) but no Logback bridge and does not manage the instrumentation artifact. Added `opentelemetry-logback-appender-1.0:2.28.0-alpha` (built on OTel 1.62.0 = Boot's SDK) and `OtlpLogBridge`, attaching the appender to the root logger after startup and detaching on close; console ECS output unchanged. RED `OtlpLogExportTests.exportsApplicationLogLinesAsOtlpLogRecords` (in-memory log exporter empty), then GREEN. Manual check (same isolated project, torn down): Loki `{service_name="frappe-api"}` returned the app's log records.
- Listener span: retried the analysis; Spring Modulith 2.1.1 builds its runtime model with `ImportOption.DoNotIncludeTests` (`ApplicationModules` source), so a test-only module listener can never be observed. **Deferred to the first business module** (end-to-end test with a real cross-module listener).

## Open questions
- Module listener span in the same trace: deferred to the first business module (Modulith ignores test classes); only the wiring is tested here.
