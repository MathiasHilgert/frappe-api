...
- Postures (`Posture`): `PUBLIC`, `AUTHENTICATED`. The HTTP layer authenticates only; authorization (RBAC per branch) belongs to the application layer (rework R1; `PERMISSION`/`SYSTEM` were removed).
- Enforcement: Spring Security (`spring-boot-starter-security`), one stateless `SecurityFilterChain`: no session, no cookies, CSRF/form login/basic/logout/request cache off. One `AuthorizationManager` per route built from its posture. Requests no annotated handler serves pass through to Spring MVC's 404/405 (rework R2); framework controllers and ambiguous matches are refused unless permitted explicitly. Public infrastructure paths: the error dispatch, `/v3/api-docs/**`, `/scalar` (only served in `local`), `/actuator/health`; other actuator endpoints are closed.
...
- OpenAPI: springdoc 3.1.1; spec at `/v3/api-docs` in every profile, the Scalar API reference (`springdoc-openapi-starter-webmvc-scalar`) only in `local` (rework R6). An `OperationCustomizer` adds the bearer requirement and documents 401 on authenticated routes.
...
- Public API (`com.frappe.platform.web`, a Modulith named interface): `@Access`, `Posture`, `SessionResolver`, `ResolvedSession`, `SessionKind`. Implementation package-private in `com.frappe.platform.infrastructure.web`.
...
- (Removed by rework R1: PERMISSION and SYSTEM postures.) Added by R2: unknown path → 404, unsupported method → 405 + `Allow`, with or without a session.
- `local` profile → the Scalar API reference is served; any other profile → not (R6: replaces swagger-ui). In both, `/v3/api-docs` shows the bearer requirement and 401 on every authenticated route.
...
### R3 async dispatches keep the caller (review MAJOR)
- Finding (review): `BearerSessionFilter` (a `OncePerRequestFilter`) only set the thread-local context, so async re-dispatches ran anonymous and an AUTHENTICATED route answered 401 after its controller ran. Verified in Spring Security 7.1.1 sources: `BearerTokenAuthenticationFilter` saves the context with a `RequestAttributeSecurityContextRepository`; with `STATELESS`, `SessionManagementConfigurer` makes that the chain's shared repository, which `SecurityContextHolderFilter` reloads on every dispatch.
- RED `RouteAccessTests.anAuthenticatedRouteAnsweringAsynchronouslyKeepsTheCallerForTheAsyncDispatch` (the route returns a `Callable`; MockMvcTester performs the async dispatch): `expected: 200 but was: 401`.
- Correction (after re-review): the R3 commit meant the filter to save the context too, but that edit never applied (spotless had re-indented the block the edit script searched for), so the repository parameter went unused. The green came only from setting the chain's `RequestAttributeSecurityContextRepository` explicitly. Experiments on the async test: removing only that chain line makes it fail again (401), and the `fb705aa` versions of the two files fail too.
- Why the chain line alone worked (Spring Security 7.1.1 sources): with an explicit repository, `SessionManagementConfigurer` gives `SessionManagementFilter` that repository instead of a `NullSecurityContextRepository`, and that filter saves a newly authenticated context. That is a side effect of session-fixation handling, not a contract.
- R8 makes the save explicit in the filter, as Spring's `BearerTokenAuthenticationFilter` does. RED `BearerSessionFilterTest.savesTheCallerInTheRequestForAsyncAndErrorDispatches`: `Expecting value to be true but was false`. GREEN: the filter calls `repository.saveContext(...)`, and `savesNothingForAnUnresolvedToken` guards the other case. The chain keeps the same explicit repository, which its `SecurityContextHolderFilter` reloads.
