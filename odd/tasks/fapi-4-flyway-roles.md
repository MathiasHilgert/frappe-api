...
- Review fix 1 RED: `DatabaseConfigurationTests.startupOutsideLocalProfileFailsWithoutDatabaseCredentials` failed: startup got past configuration and died on the DB connection (Spring's Binder passes unresolved placeholders through as literal values, so a missing password shows up only as an auth error). GREEN after the credential defaults moved to `application-local.properties`, `bootRun` activates `local` (SPRING_PROFILES_ACTIVE still wins), Spring tests use `@ActiveProfiles("local")`, and `RequiredDatabaseSettings` (an EnvironmentPostProcessor) fails fast with "Missing database setting(s) FRAPPE_... run with the 'local' profile". The assertion was tightened to that message in the same step as the implementation, so the only RED observed was the earlier failure on the DB connection.
- Review fixes 2, 4, 5: TCP healthcheck (`pg_isready -h 127.0.0.1`), persistence reference notes, FRAPPE_TEST_DB comment. No behavior test (config/docs).
