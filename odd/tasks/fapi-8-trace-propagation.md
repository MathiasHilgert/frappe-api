...
### Decisions (T0)
- **Storage**: side table `platform.event_trace_context (event_id uuid pk, traceparent, tracestate, recorded_at)` (new Flyway migration in `db/migration/platform`), written in the recording transaction by the outbox adapter, only for events selected for externalization and only when a trace is active. Commit and rollback therefore match the outbox rows. The domain event contract and Modulith's tables stay untouched. The insert uses `on conflict do nothing` and pre-validated values, so it can only fail when the database itself fails, which fails the outbox insert of the same transaction anyway. Retention (revised in review): see R5.
...
- R3 future traceparent versions: RED `acceptsAFutureVersionWithTrailingFieldsAndReadsTheFirstFour` (`Expecting Optional to contain a value but it was empty`). GREEN: version 00 stays exactly four fields; versions other than 00 and ff may append `-` plus visible ASCII, the first four fields are read (flags parsed from their fixed position), the value is passed on unchanged and capped at 512 characters. Guards `rejectsTrailingFieldsOnVersion00AndMalformedFutureVersions` (4) and `rejectsAFutureVersionLongerThan512Characters`. 23/23.
- R4 operation keys: names verified in `opentelemetry-semconv-incubating` 1.41.1-alpha sources (`messaging.operation.type`, values `create|send|receive|process|settle`; `messaging.operation.name`, system-specific, also the first word of the span name). RED `observesASuccessfulPublishWithTheSubjectAsALowCardinalityKey` and `observesProcessingAsAConsumerLinkedToTheCreationContextOfTheMessage`: `Observation should have a low cardinality tag with key <messaging.operation.type>`. GREEN: publish `send`/`publish`, process `process`/`process`, constants in `MessagingObservationKeys`. Both are bounded, so they also tag the `nats.publish` / `nats.process` timers (two more tags, no new series per value).
- R5 retention rule (docs only): the migration comment (branch-only migration, not merged, so edited in place; the reused `frappe_fapi_8` container was recreated for the new checksum) and `domain-events.md` state that a context row is purged only when its event id is in none of `event_publication`, `event_publication_archive`, `event_publication_dead_letter` (dead-letter replay needs it). The purge selects by that rule, not by age, so no index on `recorded_at`.
- R6 consumer hook (docs only): `domain-events.md` names `NatsProcessObservations` as the hook of platform's NATS subscription/inbox adapter; module consumers reach it through that adapter, and it stays package-private until the inbox ticket.
- Known deviation for the PR: all events recorded in one command share its single creation context (the span active at recording), so their messages carry the same `traceparent`; there is no per-message create span.
- Verification after R1–R6 `FRAPPE_TEST_DB=frappe_fapi_8 ./gradlew spotlessApply check --rerun-tasks`: BUILD SUCCESSFUL, 44 classes, 167 tests, 0 failures.

## Follow-ups / open questions
- Purging `platform.event_trace_context` (rule in R5) belongs to the existing follow-up that purges `platform.event_publication_archive`; until then the table grows with the outbox history.
...
- The consumer helper has no production caller yet (the NATS subscription/inbox adapter is out of scope); it is proven by a test consumer against real NATS.
