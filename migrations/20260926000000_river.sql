-- +goose Up
-- River (github.com/riverqueue/river v0.47.0) schema, main line versions
-- 002 to 007, generated with:
--
--   go run github.com/riverqueue/river/cmd/river@v0.47.0 migrate-get --all --exclude-version 1 --up
--
-- (and --down for the Down section). Version 001, River's own
-- river_migration bookkeeping table, is excluded on purpose: goose is the
-- single migration framework of this repository, so a River upgrade that
-- ships a new schema version is a new goose migration generated the same
-- way with --version <n>, never river migrate-up. Every object is created
-- by frappe_migration, so the default privileges from
-- 20260924000000_application_role_privileges.sql grant frappe_application
-- the DML River needs to insert, fetch, elect a leader and notify.
